...
// 创建类型守卫函数
function isFullHydrationLog(log: any): log is HydrationLog & { healAmount: number; buffEffect?: string } {
  return log && typeof log === 'object' && 'logType' in log && log.logType === 'HYDRATION' && 'healAmount' in log;
}
      ...
      <div v-if="currentLog" class="space-y-6 pb-safe">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
          <div class="flex items-center gap-4 mb-6">
            <div class="w-16 h-16 rounded-full bg-blue-50 dark:bg-blue-900/20 text-3xl flex items-center justify-center">
              {{ currentLog.icon }}
            </div>
            <div>
          ...
          <div class="grid grid-cols-2 gap-4 mb-6">
            <div class="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-xl text-center">
              ...
              <div class="text-xl font-black text-blue-600 dark:text-blue-300">{{ currentLog.grams || 250 }}<span class="text-sm font-normal">ml</span></div>
            </div>
            <div class="bg-indigo-50 dark:bg-indigo-900/20 p-3 rounded-xl text-center">
              ...
              <div class="text-xl font-black text-indigo-600 dark:text-indigo-300">{{ ((currentLog.grams || 250) / 250).toFixed(1) }}<span class="text-sm font-normal">杯</span></div>
            </div>
          </div>
          ...
          <div class="space-y-3">
            <div class="flex justify-between">
              ...
              <span 
                v-if="currentLog.tags && (currentLog.tags.includes('茶') || currentLog.tags.includes('咖啡') || currentLog.tags.includes('饮料'))"
                class="px-2 py-1 rounded-full text-xs font-bold"
                :class="[
                  currentLog.tags.includes('茶') ? typeLabels.TEA.bg : currentLog.tags.includes('咖啡') ? typeLabels.COFFEE.bg : typeLabels.OTHER.bg,
                  currentLog.tags.includes('茶') ? typeLabels.TEA.color : currentLog.tags.includes('咖啡') ? typeLabels.COFFEE.color : typeLabels.OTHER.color
                ]"
              >
                {{ currentLog.tags.includes('茶') ? typeLabels.TEA.label : currentLog.tags.includes('咖啡') ? typeLabels.COFFEE.label : typeLabels.OTHER.label }}
              </span>
              <span v-else class="px-2 py-1 rounded-full text-xs font-bold" :class="[typeLabels.WATER.bg, typeLabels.WATER.color]">
            ...
            <div class="flex justify-between">
              ...
              <span 
                v-if="currentLog.tags && (currentLog.tags.includes('冰镇') || currentLog.tags.includes('滚烫'))"
                class="px-2 py-1 rounded-full text-xs font-bold"
                :class="currentLog.tags.includes('冰镇') ? tempLabels.COLD.color : tempLabels.HOT.color"
              >
                {{ currentLog.tags.includes('冰镇') ? tempLabels.COLD.label : tempLabels.HOT.label }}
              </span>
              <span v-else class="text-slate-700 dark:text-slate-200">{{ tempLabels.WARM.label }}</span>
            </div>
        ...
        <!-- RPG 效果 -->
        <div v-if="!systemStore.isPureMode && isFullHydrationLog(currentLog) && (currentLog.healAmount || currentLog.buffEffect)" class="bg-gradient-to-br from-cyan-50 to-blue-50 dark:from-slate-800 dark:to-slate-700 rounded-2xl p-6 shadow-sm border border-cyan-100 dark:border-slate-600">
          <h4 class="font-bold text-lg mb-4 text-slate-800 dark:text-white flex items-center">
            <span class="text-2xl mr-2">💧</span>
            RPG 效果
          </h4>
          <div class="space-y-3">
            <div v-if="currentLog.healAmount" class="flex items-center justify-between bg-white/60 dark:bg-slate-800/60 rounded-xl p-3">
            ...
            <div v-if="currentLog.buffEffect" class="flex items-center justify-between bg-white/60 dark:bg-slate-800/60 rounded-xl p-3">
        ...
        <!-- 健康收益 - 仅RPG模式显示 -->
        <div v-if="!systemStore.isPureMode" class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-2xl p-6 shadow-sm border border-blue-100 dark:border-blue-800">
          <h4 class="font-bold text-lg mb-4 text-slate-800 dark:text-white flex items-center">
            <span class="text-2xl mr-2">💦</span>
            健康收益
          </h4>
          <div class="space-y-3">
            <div class="flex items-center justify-between bg-white/60 dark:bg-slate-800/60 rounded-xl p-3">
              ...
              <span class="font-bold text-blue-500">{{ (currentLog.grams || 250) >= 500 ? '显著' : (currentLog.grams || 250) >= 250 ? '良好' : '轻微' }}</span>
            </div>
            <div class="flex items-center justify-between bg-white/60 dark:bg-slate-800/60 rounded-xl p-3">
              <div class="flex items-center gap-2">
                <span class="text-xl">🧠</span>
              ...
              <span class="font-bold text-purple-500">+{{ Math.round((currentLog.grams || 250) / 50) }}%</span>
            </div>
            <div class="flex items-center justify-between bg-white/60 dark:bg-slate-800/60 rounded-xl p-3">
              <div class="flex items-center gap-2">
                <span class="text-xl">✨</span>
              ...
              <span class="font-bold text-pink-500">+{{ Math.round((currentLog.grams || 250) / 100) }}%</span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
          ...
          <div class="space-y-3">
            <div class="flex justify-between items-center">
              ...
              <span class="font-bold text-slate-700 dark:text-slate-200">~{{ Math.round(((currentLog.grams || 250) / 2000) * 100) }}%</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-slate-600 dark:text-slate-400 text-sm">相当于标准杯</span>
              <span class="font-bold text-slate-700 dark:text-slate-200">{{ ((currentLog.grams || 250) / 250).toFixed(1) }} 杯</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-slate-600 dark:text-slate-400 text-sm">体液补充</span>
              <span class="font-bold text-slate-700 dark:text-slate-200">{{ ((currentLog.grams || 250) / 50).toFixed(0) }}ml 血液</span>
            </div>
          </div>
        </div>

        <!-- 健康小贴士 -->
        <div class="bg-gradient-to-br from-amber-50 to-yellow-50 dark:from-amber-900/20 dark:to-yellow-900/20 rounded-2xl p-6 shadow-sm border border-amber-100 dark:border-amber-800">
          ...
          <div class="text-sm text-slate-600 dark:text-slate-300 space-y-2">
            <p class="flex items-start gap-2">
              ...
              <span>{{ currentLog.tags?.includes('茶') ? '茶含有抗氧化物质，有益健康' : currentLog.tags?.includes('咖啡') ? '适量咖啡可提神，但不宜过量' : currentLog.tags?.includes('饮料') ? '注意控制饮料中的糖分摄入' : '纯净水是最佳选择，不含糖分和热量' }}</span>
            </p>
            <p class="flex items-start gap-2">
              <span class="text-amber-500 shrink-0">•</span>
              <span>{{ currentLog.tags?.includes('冰镇') ? '冰饮会刺激肠胃，不宜空腹饮用' : currentLog.tags?.includes('滚烫') ? '热饮可促进血液循环' : '温水最适合身体吸收' }}</span>
            </p>
            <p class="flex items-start gap-2">
              <span class="text-amber-500 shrink-0">•</span>
