...
// 创建类型守卫函数
function isFullExerciseLog(
  log: unknown,
): log is ExerciseLog & { healAmount: number; shieldGained: number; goldGained: number } {
  return !!(
    log &&
    typeof log === 'object' &&
    'logType' in log &&
    (log as { logType?: string }).logType === 'EXERCISE' &&
    'healAmount' in log
  )
}
...
const deleteLog = () => {
  ...
  const removed = exerciseStore.removeExercise(currentLog.value.id)
  if (removed) {
    showToast('记录已删除')
    router.back()
  }
      ...
      <div v-if="currentLog" class="space-y-6 pb-safe">
        ...
        >
          ...
          <div class="grid grid-cols-3 gap-4 mb-6">
            <div class="bg-orange-50 dark:bg-orange-900/20 p-3 rounded-xl text-center">
              ...
              <div class="text-xl font-black text-orange-600 dark:text-orange-300">
                {{ currentLog.duration || 0 }}<span class="text-sm font-normal">分钟</span>
              </div>
            </div>
            <div class="bg-red-50 dark:bg-red-900/20 p-3 rounded-xl text-center">
              ...
              <div class="text-xl font-black text-red-600 dark:text-red-300">
                {{ currentLog.caloriesBurned || 0 }}<span class="text-sm font-normal">kcal</span>
              </div>
            </div>
            <div
              v-if="
                ...
                (currentLog.tags.includes('高强度') ||
                  ...
                  currentLog.tags.includes('中强度'))
              "
              class="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-xl text-center"
            >
              <div class="text-xs text-purple-600 dark:text-purple-400 font-bold mb-1">强度</div>
              <div class="text-xl font-black text-purple-600 dark:text-purple-300">
                {{
                  currentLog.tags.includes('高强度')
                    ? intensityLabels.HIGH.label
                    : currentLog.tags.includes('低强度')
                      ? intensityLabels.LOW.label
                      : intensityLabels.MEDIUM.label
                }}
              </div>
            </div>
          </div>
        ...
        <div
          v-if="
            ...
            isFullExerciseLog(currentLog) &&
            (currentLog.healAmount || currentLog.shieldGained || currentLog.goldGained)
          "
          class="bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-slate-800 dark:to-slate-700 rounded-2xl p-6 shadow-sm border border-purple-100 dark:border-slate-600"
        >
          ...
          <div class="grid grid-cols-3 gap-4">
            <div
              ...
              class="text-center bg-white/60 dark:bg-slate-800/60 rounded-xl p-4"
            >
        ...
        <div
          v-if="
            !systemStore.isPureMode &&
            ('goldGained' in currentLog || 'generatedExp' in currentLog) &&
            (currentLog.goldGained || currentLog.generatedExp)
          "
        ...
        <div
          ...
          class="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-2xl p-6 shadow-sm border border-green-100 dark:border-green-800"
        >
          <h4 class="font-bold text-lg mb-4 text-slate-800 dark:text-white flex items-center">
            <span class="text-2xl mr-2">💪</span>
            健康收益
