...
// 获取装备槽位配置（假设 store 中已定义 getter: equipmentList）
// 如果 store 中没有，需要像之前那样在这里重新定义一遍逻辑
const equipment = computed(() => {
  // 简易版 Mock，建议移至 store getter
  ...
  return slotDefinitions.map(def => {
    ...
    // @ts-ignore: 假设 achievements 在 store 中
...
const saveProfile = () => {
  ...
  // 假设 store 有 recalcBMR action
  // store.recalcBMR();
  // 简单实现 BMR 更新逻辑
  const s = 5;
  const bmr = 10 * store.user.weight + 6.25 * store.user.height - 5 * store.user.age + s;
  store.user.baseBMR = Math.round(bmr * 1.375);
  store.saveState();
...
<template>
  <div class="pb-24 bg-slate-900 min-h-full text-white">
    ...
    <div class="mt-16 text-center px-6">
      ...
      <p class="text-xs text-slate-400 mt-1">传说中的卡路里猎人</p>
    ...
    <!-- 编辑弹窗 (局部) -->
    <van-dialog v-model:show="showEdit" title="修改档案" show-cancel-button @confirm="saveProfile" class="dark:bg-slate-800 dark:text-white">
