...
// 判断数据类型
const isRPGData = computed(() => point.value && isRPGTrendData(point.value));
const isPureData = computed(() => point.value && isPureTrendData(point.value));
...
<template>
  <van-popup v-model:show="show" round position="center" 
    ...
    <div v-if="point" class="overflow-y-auto max-h-[85vh] custom-scrollbar">
      ...
      <!-- RPG模式详情 -->
      <div v-if="isRPGData && !isPure" class="rpg-detail p-6">
        <!-- 顶部装饰 -->
        ...
        <div class="bg-gradient-to-br from-slate-900/20 to-slate-800/20 rounded-xl p-4 border border-slate-600 mb-4">
          <div class="flex items-center justify-between">
            ...
            <div class="text-3xl font-black text-yellow-400">{{ point.weight }} <span class="text-lg font-normal">kg</span></div>
          </div>
        </div>
        
        <!-- 属性变化 -->
        <div v-if="point.attributeChanges" class="bg-gradient-to-br from-purple-900/20 to-blue-900/20 rounded-xl p-4 border border-purple-500/30 mb-4">
          ...
          <div class="grid grid-cols-3 gap-3">
            <div class="text-center bg-red-900/20 border border-red-500/30 rounded-lg p-3">
              ...
              <div class="text-lg font-bold" :class="point.attributeChanges.str >= 0 ? 'text-red-300' : 'text-red-500'">
                {{ point.attributeChanges.str > 0 ? '+' : '' }}{{ point.attributeChanges.str }}
              </div>
            </div>
            <div class="text-center bg-green-900/20 border border-green-500/30 rounded-lg p-3">
              ...
              <div class="text-lg font-bold" :class="point.attributeChanges.agi >= 0 ? 'text-green-300' : 'text-green-500'">
                {{ point.attributeChanges.agi > 0 ? '+' : '' }}{{ point.attributeChanges.agi }}
              </div>
            </div>
            <div class="text-center bg-blue-900/20 border border-blue-500/30 rounded-lg p-3">
              ...
              <div class="text-lg font-bold" :class="point.attributeChanges.vit >= 0 ? 'text-blue-300' : 'text-blue-500'">
                {{ point.attributeChanges.vit > 0 ? '+' : '' }}{{ point.attributeChanges.vit }}
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- 战力变化 -->
        <div v-if="point.combatPowerChange !== undefined" class="bg-gradient-to-r from-yellow-900/20 to-orange-900/20 rounded-xl p-4 border border-yellow-500/30 mb-4">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-2xl">⚔️</span>
              <span class="text-sm font-bold text-yellow-300">战力变化</span>
            </div>
            <div class="text-2xl font-black" :class="point.combatPowerChange >= 0 ? 'text-yellow-400' : 'text-orange-500'">
              {{ point.combatPowerChange > 0 ? '+' : '' }}{{ point.combatPowerChange }}
            </div>
          </div>
        </div>
        
        <!-- 成就解锁 -->
        <div v-if="point.achievements && point.achievements.length > 0" class="bg-yellow-900/20 rounded-xl p-4 border border-yellow-500/30 mb-4">
          <h4 class="text-xs font-bold text-yellow-300 mb-3 uppercase tracking-wider">🏆 成就解锁</h4>
          <div class="space-y-2">
            <div v-for="(ach, idx) in point.achievements" :key="idx" 
        ...
        <!-- 里程碑 -->
        <div v-if="point.milestones && point.milestones.length > 0" class="bg-slate-800/50 rounded-xl p-4 border border-slate-600 mb-4">
          <h4 class="text-xs font-bold text-slate-300 mb-3 uppercase tracking-wider">📍 里程碑</h4>
          <div class="space-y-2">
            <div v-for="(ms, idx) in point.milestones" :key="idx" 
        ...
        <!-- 故事节点 -->
        <div v-if="point.storyNode" class="bg-slate-900/50 rounded-xl p-4 border border-slate-700">
          <div class="flex items-start gap-3">
            <span class="text-3xl">📜</span>
            <div class="flex-1">
              <div class="text-xs text-slate-400 mb-1 uppercase tracking-wider">冒险日志</div>
              <div class="text-sm text-slate-200 leading-relaxed">{{ point.storyNode }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- 纯净模式详情 -->
      <div v-else-if="isPureData && isPure" class="pure-detail p-6">
        <div class="text-center mb-6">
        ...
        <div class="grid grid-cols-2 gap-3 mb-4">
          <div class="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border border-blue-200 dark:border-blue-800">
            ...
            <div class="text-3xl font-bold text-slate-800 dark:text-white">
              {{ point.weight }} <span class="text-lg font-normal text-slate-500">kg</span>
            </div>
          </div>
          
          <div class="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 border border-green-200 dark:border-green-800">
            <div class="text-xs text-slate-500 dark:text-slate-400 mb-1">BMI</div>
            <div class="text-3xl font-bold" :class="getBMIStatus(point.bmi).color">
              {{ point.bmi.toFixed(1) }}
            </div>
            <div class="text-[10px] text-slate-400 mt-1">{{ getBMIStatus(point.bmi).text }}</div>
          </div>
          
          <div v-if="point.changeRate !== undefined" class="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4 border border-purple-200 dark:border-purple-800">
            <div class="text-xs text-slate-500 dark:text-slate-400 mb-1">周变化率</div>
            <div class="text-xl font-bold" :class="Math.abs(point.changeRate * 7) < 0.5 ? 'text-green-600' : 'text-orange-600'">
              {{ (point.changeRate * 7) > 0 ? '+' : '' }}{{ (point.changeRate * 7).toFixed(2) }} kg
            </div>
            <div class="text-[10px] text-slate-400 mt-1">每周</div>
          </div>
          
          <div v-if="point.healthScore !== undefined" class="bg-orange-50 dark:bg-orange-900/20 rounded-xl p-4 border border-orange-200 dark:border-orange-800">
            <div class="text-xs text-slate-500 dark:text-slate-400 mb-1">健康评分</div>
            <div class="text-xl font-bold text-orange-600">
              {{ point.healthScore }}<span class="text-sm font-normal">/100</span>
            </div>
            <div class="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-full mt-2">
              <div class="h-full rounded-full transition-all"
                   :class="point.healthScore >= 80 ? 'bg-green-500' : point.healthScore >= 60 ? 'bg-orange-500' : 'bg-red-500'"
                   :style="{ width: point.healthScore + '%' }"></div>
            </div>
          </div>
        </div>
        
        <!-- 专业指标 -->
        <div v-if="point.professionalMetrics" class="bg-slate-50 dark:bg-slate-800 rounded-xl p-4 border border-slate-200 dark:border-slate-700 mb-4">
          ...
          <div class="space-y-3">
            <div class="flex justify-between items-center">
              ...
              <span class="text-sm font-bold text-slate-700 dark:text-slate-200">{{ point.professionalMetrics.weeklyAverage }} kg</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-xs text-slate-500 dark:text-slate-400">月度趋势</span>
              <span class="text-sm font-bold" :class="{
                'text-red-500': point.professionalMetrics.monthlyTrend === 'UP',
                'text-green-500': point.professionalMetrics.monthlyTrend === 'DOWN',
                'text-blue-500': point.professionalMetrics.monthlyTrend === 'STABLE'
              }">
                {{ point.professionalMetrics.monthlyTrend === 'UP' ? '上升 ↗' : 
                   point.professionalMetrics.monthlyTrend === 'DOWN' ? '下降 ↘' : '稳定 →' }}
              </span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-xs text-slate-500 dark:text-slate-400">距离目标</span>
              <span class="text-sm font-bold" :class="point.professionalMetrics.targetDiff > 0 ? 'text-orange-500' : 'text-green-500'">
                {{ Math.abs(point.professionalMetrics.targetDiff).toFixed(1) }} kg
                {{ point.professionalMetrics.targetDiff > 0 ? '(需减重)' : '(已达标)' }}
              </span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-xs text-slate-500 dark:text-slate-400">波动性</span>
              <span class="text-sm font-bold" :class="point.professionalMetrics.volatility < 0.5 ? 'text-green-500' : 'text-orange-500'">
                {{ point.professionalMetrics.volatility.toFixed(2) }} kg
                {{ point.professionalMetrics.volatility < 0.5 ? '(稳定)' : '(波动较大)' }}
              </span>
            </div>
          </div>
        </div>
        ...
        <!-- 数据洞察 -->
        <div v-if="point.insights && point.insights.length > 0" class="space-y-2 mb-4">
          <h4 class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">💡 当日洞察</h4>
          <div v-for="(insight, idx) in point.insights" :key="idx"
