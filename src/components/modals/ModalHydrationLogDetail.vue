...
<template>
  <van-popup
    ...
    class="dark:bg-slate-800 flex flex-col overflow-hidden"
  >
    <div class="p-6 text-center overflow-y-auto custom-scrollbar" v-if="log">
      ...
      <!-- RPG效果卡片 -->
      <div
        class="bg-gradient-to-br from-cyan-900/20 to-blue-900/20 dark:from-cyan-900/30 dark:to-blue-900/30 rounded-xl p-4 mb-4 border border-cyan-500/30"
      >
        <div class="text-xs text-cyan-400 font-bold mb-2 uppercase tracking-wider">💦 净化效果</div>
        <div class="grid grid-cols-2 gap-3">
          <div class="text-center">
            <div class="text-2xl mb-1">💧</div>
            <div class="text-lg font-bold text-blue-400">+{{ log.amount || 250 }}</div>
            <div class="text-[10px] text-slate-400">ml 水分</div>
          </div>
          <div class="text-center">
            <div class="text-2xl mb-1">✨</div>
            <div class="text-lg font-bold text-cyan-400">
              {{ Math.round(((log.amount || 250) / 2000) * 100) }}%
            </div>
      ...
      <div class="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-4 mb-4 space-y-3">
        <div class="flex justify-between items-center">
          ...
          <span class="font-bold text-blue-500">{{ log.amount || 250 }} ml</span>
        </div>
        <div class="flex justify-between items-center" v-if="log.tags && log.tags.length > 0">
          ...
          <span
            ...
            :class="
              typeLabels[
                log.tags.includes('茶') ? 'TEA' : log.tags.includes('咖啡') ? 'COFFEE' : 'WATER'
              ]?.color
            "
          >
            {{
        ...
        <div
          ...
          v-if="log.tags && log.tags.some((t) => ['冰镇', '温热', '滚烫'].includes(t))"
        >
          <span class="text-xs text-slate-400">🌡️ 水温</span>
          <span class="font-bold text-slate-600 dark:text-slate-300">
            {{
              tempLabels[
                log.tags.includes('冰镇') ? 'COLD' : log.tags.includes('滚烫') ? 'HOT' : 'WARM'
              ]?.icon
            }}
            {{
              tempLabels[
      ...
      <!-- 补水收益 -->
      <div
        class="bg-gradient-to-br from-indigo-900/20 to-purple-900/20 dark:from-indigo-900/30 dark:to-purple-900/30 rounded-xl p-4 mb-4 border border-indigo-500/30"
      >
        <div class="text-xs text-indigo-400 font-bold mb-3 uppercase tracking-wider">
          📊 冒险收益
        </div>
      ...
      <div
        v-if="!systemStore.isPureMode && (log.generatedGold || log.generatedExp)"
        class="bg-gradient-to-br from-blue-900/20 to-cyan-900/20 dark:from-blue-900/30 dark:to-cyan-900/30 rounded-xl p-4 mb-4 border border-blue-500/30"
      >
      ...
      <!-- 健康提示 -->
      <div
        class="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-3 mb-4 text-left border border-amber-200 dark:border-amber-800/30"
      >
        ...
        <div class="text-xs text-slate-600 dark:text-slate-300">
          {{
            log.tags?.includes('茶')
              ? '茶含有抗氧化物质，有益健康'
              : log.tags?.includes('咖啡')
                ? '适量咖啡可提神，但不宜过量'
                : log.tags?.includes('饮料')
                  ? '注意控制饮料中的糖分摄入'
                  : '纯净水是最佳选择，不含糖分和热量'
          }}
        </div>
