...
<template>
  <div v-if="show" class="fixed inset-0 z-[9990] font-sans">
    ...
    <div class="guide-spotlight transition-all duration-300 ease-out"
    ...
    <!-- 2. 全屏遮罩 -->
    <!-- 降低不透明度，让应用看起来更通透 -->
    <div v-if="!currentStep?.focusId" class="absolute inset-0 bg-slate-900/60 backdrop-blur-[2px] transition-all duration-500"></div>

    <!-- 3. 交互层容器 -->
    <div class="fixed inset-0 pointer-events-none z-[9995] flex flex-col justify-end sm:justify-center">

      <!-- Dialog Card -->
      <!-- pointer-events-auto: 恢复交互 -->
      <!-- safe-area-bottom: 适配 iPhone X -->
      <div class="pointer-events-auto relative w-full sm:w-[90%] sm:rounded-3xl bg-white dark:bg-slate-800 shadow-2xl transition-all duration-500 ease-out overflow-hidden flex flex-col"
           :class="[
        ...
        <!-- 装饰背景：顶部的微弱渐变光晕，增加质感 -->
        <div class="absolute top-0 inset-x-0 h-32 opacity-30 pointer-events-none"
        ...
        <div class="px-6 pt-6 flex items-center justify-between relative z-10">
          <div class="flex items-center gap-3">
            <!-- NPC Avatar Box -->
            <div class="w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-sm border border-slate-100 dark:border-slate-700"
                 :class="theme.bgLight">
              {{ npc.icon }}
            </div>
            <div>
              <div class="flex items-center gap-2">
                <span class="font-bold text-slate-800 dark:text-slate-100 text-lg">{{ npc.name }}</span>
                <!-- Badge -->
              ...
              <p class="text-xs text-slate-400 font-medium">Step {{ currentStepIndex + 1 }} / {{ guideSteps.length }}</p>
            </div>
          </div>
        ...
        <div class="px-6 py-4 relative z-10 min-h-[100px]">
          ...
          <p class="text-base leading-relaxed whitespace-pre-line text-slate-600 dark:text-slate-300">
            {{ currentStep?.text }}
          </p>
        </div>
