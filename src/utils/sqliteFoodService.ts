...
class SqliteFoodService {
   ...
   * 搜索资料库
   */
  async searchLibrary(query: string, filter: { type?: string, category?: string } = {}): Promise<Record<string, unknown>[]> {
    await this.init();
    ...
    if (this.isNative && this.dbConnection) {
      ...
      return new Promise((resolve) => {
        ...
        request.onsuccess = () => {
          ...
          const results = all.filter((item: Record<string, unknown>) => {
          ...
          resolve(results.map((row: Record<string, unknown>) => {
            try { return JSON.parse(row.json_data); } catch { return null; }
          }).filter(Boolean));
        };
      });
