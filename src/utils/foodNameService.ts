...
type RaceVocab = { general: VocabSet } & Partial<Record<string, VocabSet>>

const CULTURAL_FLAVOR: Record<string, RaceVocab> = {
  // ================= HUMAN (人类) =================
  HUMAN: {
    general: {
      origin: [
  ...
  ORC: {
    ...
    general: {
      origin: ['贫瘠之地', '雷霆崖', '回音群岛', '杜隆塔尔', '森金村', '棘齿城', '试炼谷'],
      material: ['粗糙', '坚硬', '带刺', '荆棘', '仙人掌', '巫毒', '萨满', '图腾', '苦工'],
      prefix: ['战地', '掠夺', '野战', '行军', '军团', '氏族', '力量', '幸存者'],
  ...
  ELF: {
    ...
    fruit: {
      ...
      prefix: ['发光的', '纯净的', '世界树的', '低语的', '神圣的', '禁忌的', '晶莹的'],
  ...
  DWARF: {
    ...
    general: {
      origin: ['丹莫罗', '洛克莫丹', '湿地', '暮光高地', '格瑞姆巴托', '荒芜之地'],
      material: ['岩石', '矿石', '花岗岩', '玄武岩', '蒸汽', '机械', '火药', '符文', '工匠'],
      prefix: ['烟熏', '石化', '坚固的', '风干', '腌制', '油炸', '高热量', '矿工'],
...
// 从数组中安全获取元素
const pick = (arr: string[], rand: number): string => {
...
export const getFoodDisplayName = (
  food: FoodItem,
  isRpgMode: boolean,
  userRace: string = 'HUMAN',
): string => {
  if (!isRpgMode) return food.originalName || food.name

  // 1. 获取词库

  const safeHumanConfig =
    CULTURAL_FLAVOR.HUMAN ??
    CULTURAL_FLAVOR.ELF ??
    CULTURAL_FLAVOR.ORC ??
    CULTURAL_FLAVOR.DWARF ??
    ({ general: { origin: [], material: [], prefix: [], suffix: [] } } as RaceVocab)

  const raceConfig = CULTURAL_FLAVOR[userRace] ?? safeHumanConfig
  const category = getCategory(food.tags || [])
  const vocab = raceConfig[category] ?? raceConfig.general
  ...
  if (isLongName) {
    ...
    if (randPattern < 0.3) {
      ...
      // 如果基名里已经包含了材质词，就只用地名
      rpgName = baseName.includes(material)
        ? `${origin}特产·${baseName}`
        : `${origin}·${material}${baseName}`
...
export const getFoodDisplayDesc = (
  food: FoodItem,
  isRpgMode: boolean,
  userRace: string = 'HUMAN',
): string => {
  ...
  if (userRace === 'ORC') {
    if (category === 'meat')
      return rand > 0.5 ? '充满野性的肉块，能极大地补充怒气！' : '为了部落！吃饱了才有力气战斗。'
